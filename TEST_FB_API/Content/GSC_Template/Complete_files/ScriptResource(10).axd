...
},dataBind:function(g,d){var f=this._dataSource;
if(!a.isArray(f)){return;
}if(d===f.length){d=0;
}this._isDataBinding=true;
if(!g){var e=this._buildEventArgs(new Sys.CancelEventArgs(),{isAppendingData:!!d});
this.raise_dataBinding(e);
if(e.get_cancel()){this._isDataBinding=false;
return;
}}if(!d){this._clearRenderedHtml();
this._createLayout();
if(!f.length){this._createEmptyItem();
}}var k=!!d?f.length-d:0;
for(var h=k,j=f.length;
h<j;
h++){if(h===k&&k>0){this._createSeparatorItem();
}this._createDataItem(f[h],h);
if(h<j-1){this._createSeparatorItem();
}}this._isDataBinding=false;
...
h=a.proxy(f.append,f);
}if(typeof(g)!=="undefined"){h(document.createComment(String.format(this._itemStartCommentFormat,this.get_id(),g)));
h(i);
